...
static timing_t benchmark_afl_spinlock(size_t iters, benchmark_split *split)
{
    BENCH_LOCK_UNLOCK(iters, afl_spin_lock(&afl_spinlock), afl_spin_unlock(&afl_spinlock), split);
...
int main(void)
{
    ...
    benchmark_info afl_spinlock     = {.name = "afl", .func = benchmark_afl_spinlock};

    do_bench(&pthread_spinlock);
    do_bench(&afl_spinlock);

    print_benchmark(afl_spinlock, pthread_spinlock);
